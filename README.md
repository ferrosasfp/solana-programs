...
| On-chain IDL | **Published** on 2026-08-01, in the canonical metadata account `7tbJDv1gwseQamg816gEgwTSpsPpgec5yxhYpbTrcdbC`. `anchor idl fetch` returns it and its canonical sha256 matches this repository and both consumers. The explorer may still not render Anchor IDLs; fetching works regardless. See [Publishing the IDL on chain](doc/publish-idl-onchain.md), including why the documented command fails and what worked instead. **⚠️ Pending re-pin as of 2026-08-04:** a `///` doc comment on the mint was corrected in this tree because it claimed a compensating control that does not exist. Anchor copies `///` comments into the IDL, so the next `anchor build` produces a different canonical sha256 than the one published on chain and pinned by both consumers. The binary is unchanged (the diff is comments only). Until someone rebuilds, republishes and re-pins, source and published IDL differ by that one paragraph, and that is a deliberate, declared gap: leaving a false statement inside a published artifact is worse than a hash that has to move. |
